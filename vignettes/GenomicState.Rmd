...
This is how you can query the data from `GenomicState` using `r citep(bib[['AnnotationHub']])`.

```{r 'annotation_hub'}
library('AnnotationHub')
ah <- AnnotationHub()
q <- query(ah, 'GenomicState')

## Waiting for data to be there before finishing this part
q
```
...
```{r 'build_ex_objects'}
## Note: should probably change the example so it doesn't need to be
## computed...

## Load the example TxDb object
## or start from scratch with:
## txdb_v31_hg38_chr22 <- gencode_txdb(version = '31', genome = 'hg38',
##     chrs = 'chr22')
...
## Build the TxDb object for chr21 for hg19
...
Next we load a series of related packages that use these objects.
...
The objects provided by `GenomicState` through `r citep(bib[['AnnotationHub']])` were built using code like the one included below which is how the Gencode version 23 for hg19 files were built.
...
system.file('scripts', 'make-data_gencode_human.R', package = 'GenomicState')
## The above file was created by this one:
system.file('scripts', 'generate_make_data_gencode_human.R', package = 'GenomicState')
```
